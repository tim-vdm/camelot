...
def deploy_new_certificate(domain='patronale'):
    with context_managers.settings(host_string=env.HOST_NAME,
                                   user=env.HOST_USER,
                                   key_filename='../conf/{0}.pem'.format(env.CONFIGURATION)):
        try:
            api.put('../conf/{}_ssl.crt'.format(domain), '/opt/ssl/{}_ssl.crt'.format(domain), use_sudo=True)
            api.put('../conf/{}_ssl.key'.format(domain), '/opt/ssl/{}_ssl.key'.format(domain), use_sudo=True)
        except ValueError as ve:
            print('*** You must generate the key and crt files. Please see ../conf/README. ***')
            raise ve
        api.sudo('service nginx reload')
...
def generate_db_file(db_name='generated'):
    with context_managers.settings(host_string=env.HOST_NAME,
        ...
        for d, _, filenames in os.walk(os.path.join('..', 'sql')):
            for sql_file in sorted([filename for filename in filenames if filename.endswith('sql')]):
        ...
        cursor.execute("INSERT INTO financial_agreement (row_type, code, agreement_date, apply_from_date, apply_thru_date, package_id, version_id) VALUES ('financial_agreement', '{}', '2001-01-01', '2001-01-01', '2400-12-31', 65, 1);".format(last_agreement_code))
        connection.commit()
        connection.close()
