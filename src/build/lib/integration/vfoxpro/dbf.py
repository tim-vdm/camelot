...
import logging
logger = logging.getLogger('libraries.integration.vfoxpro.dbf')
...
class Dbase:
    ...
    def _reverse_endian(self, num):
        if not len(num):
            return 0
        if struct.calcsize('<L')!=len(num):
          logger.warn('needed size : %s, actual size : %s'%(struct.calcsize('<L'), len(num)))
          logger.warn("%s '%s'"%(len(num), num))
          num = num[-4:]
